...
class Framework:
    ...
    def get_nbr(self, sample, config, distance="Euc"):
        json = neighborhood_configs[config]
        nbr_json = {"no_of_neighbours": 1000, "bound": True, "truly_random": json["random"]}
        e = self.eval_obj
        pred = e.clf.predict([sample[e.features]])[0]
        if json["training"]:
            neighbors = e.traindf[e.features]
        else:
            neighbors = e.context.generate_neighbourhood([], sample, self.eval_obj.features,
                                                         nbr_json["no_of_neighbours"],
                                                         False, True, False, nbr_json["truly_random"])
